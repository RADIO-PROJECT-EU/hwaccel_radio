...
#define DMA0_BASE_ADDR 0x43000000
#define EQUALIZE_HIST_BASE_ADDR 0x43C00000

#define IN_IMG_ADDR    		0x1F400000

#define MAX_IMAGE_WIDTH  	800
#define MAX_IMAGE_HEIGHT 	600
#define IN_IMAGE_CHANNELS	3
#define OUT_IMAGE_CHANNELS	1
#define MAX_IMAGE_SIZE 		MAX_IMAGE_WIDTH * MAX_IMAGE_HEIGHT * IN_IMAGE_CHANNELS
...
/*	if(argc > 1){
		config = (int)strtol(argv[1], NULL, 10);
	}
*/
...
//		resetDMA(vdma_ptr);		
		if(config == 0 || config == 1){
			configDMA(vdma_ptr, IN_IMG_ADDR, OUT_IMG_ADDR,
				inImage.rows, inImage.cols*IN_IMAGE_CHANNELS,
				outImage.rows, outImage.cols*OUT_IMAGE_CHANNELS);
		...
		cout << "setSize: " << grayEqualize.getFrameWidth() << "x" << grayEqualize.getFrameHeight() << endl;
		if(config == 0 || config == 2){
			grayEqualize.start();
		}
		//bool done = false;
		unsigned int ctrl = grayEqualize.getControlRegister();
		cout << intToBin(ctrl) << endl;
		while( (ctrl&2) != 2){
//			sleep(1);
			usleep(1000*500);
			ctrl = grayEqualize.getControlRegister();
			cout << intToBin(ctrl) << endl;
		}
		//sleep(1);
